    ...
    <motion.div className='bio-container'>
      ...
      <div className='contenidoCV'>
        ...
        {seccionActiva === 2 && (
          <motion.div initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 1.5 }} className='infCVPro'>
            {
              proyectos.map((proyecto) => (
                <div className='card-cont'>
                  <h2>{proyecto.nombre}</h2>
                  <p>Descripción: {proyecto.descripcion}</p>
                  <p>GitHub Frontend:<a href={proyecto.gitHubFrontend} target='blank'> {proyecto.gitHubFrontend}</a></p>
                  {
                    proyecto.gitHubBackend !== 'Sin backend.' ? (
                      <p>GitHub backend: <a href={proyecto.gitHubBackend} target='blank'> {proyecto.gitHubBackend}</a></p>
                    ) : (
                      <p>GitHub backend: {proyecto.gitHubBackend}</p>
                    )
                  }
                  {proyecto.urlProyecto === 'Próximamente.' ? (<p>Url del proyecto: {proyecto.urlProyecto }</p>) : (<p>Url del proyecto: <a href={proyecto.urlProyecto } target='_blank' rel='noopener noreferrer'> {proyecto.urlProyecto }</a></p>)}
                  <p>Tecnologías: {proyecto.tecnologías}</p>
